...
hivex_value_data_cell_offset (hive_h *h, hive_value_h value, size_t *len)
{
  ...
  if (h->msglvl >= 2)
    fprintf (stderr, "hivex_data_cell_offset: value=0x%zx\n", value);
  struct ntreg_vk_record *vk = (struct ntreg_vk_record *) (h->addr + value);
  ...
  if (h->msglvl >= 2)
    fprintf (stderr, "hivex_data_cell_offset: is_inline=%d\n", is_inline);

  if (h->msglvl >= 2)
    fprintf (stderr, "hivex_data_cell_offset: data_len=%zx\n", data_len);
  ...
  if (h->msglvl >= 2)
    fprintf (stderr, "hivex_data_cell_offset: data_offset=%zx\n", data_offset);
