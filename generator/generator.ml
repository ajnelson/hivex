...
static value copy_value_len (hive_value_h, size_t);
  ...
  List.iter (
	...
	| ASetValue ->
      ...
      (match fst style with
       ...
       | RLenValue -> pr "  rv = copy_value_len (r, len);\n"
       | RLenTypeVal ->
...
static value
copy_value_len (hive_value_h r, size_t len)
{
  CAMLparam0 ();
  CAMLlocal2 (v, rv);
...
" call args ret shortdesc doc name name in
	...
	| RHive -> pr "  hive_h *r;\n"; "NULL"
      ...
      | RLenValue ->
        pr "  VALUE rv = rb_hash_new ();\n";
        pr "  rb_hash_aset (rv, ID2SYM (rb_intern (\"len\")), INT2NUM (len));\n";
        pr "  rb_hash_aset (rv, ID2SYM (rb_intern (\"value\")), ULL2NUM (r));\n";
        pr "  return rv;\n"
      | RLenTypeVal ->
        pr "  VALUE rv = rb_hash_new ();\n";
        pr "  rb_hash_aset (rv, ID2SYM (rb_intern (\"len\")), INT2NUM (len));\n";
